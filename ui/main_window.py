...
class MainWindow(PlaybackMixin, CacheMixin, PipelineMixin, ExecutionMixin, QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # 音色库 (原声字幕行右侧)
        grid.addWidget(QLabel("男声:"), 2, 3)
        self.speaker_emb_male = _AutoRefreshCombo(self)
        self.speaker_emb_male.setMinimumWidth(120)
        grid.addWidget(self.speaker_emb_male, 2, 4)
        grid.addWidget(QLabel("女声:"), 2, 5)
        self.speaker_emb_female = _AutoRefreshCombo(self)
        self.speaker_emb_female.setMinimumWidth(120)
        grid.addWidget(self.speaker_emb_female, 2, 6)
        _btn_train = QPushButton("训练")
        _btn_train.setFixedWidth(50)
        _btn_train.clicked.connect(self._train_speaker_embedding)
        grid.addWidget(_btn_train, 2, 7)
    ...
    def _train_speaker_embedding(self):
        """用当前筛选显示的字幕训练说话人嵌入"""
        if not self.current_subtitles:
            QMessageBox.warning(self, "训练音色", "没有字幕数据")
            return
        subs = []
        ...
        if _work_dir:
            _work_dir = _work_dir.work_dir
        if not _work_dir or not os.path.isdir(_work_dir):
            _work_dir = getattr(getattr(self, '_single_step_worker', None), 'ctx', None)
            if _work_dir:
                _work_dir = _work_dir.work_dir
        if not _work_dir:
            _work_dir = os.path.join(tempfile.gettempdir(), "speaker_emb_train")
        tmp_dir = os.path.join(_work_dir, "speaker_emb_train")
        if os.path.isdir(tmp_dir):
            shutil.rmtree(tmp_dir, ignore_errors=True)
        os.makedirs(tmp_dir, exist_ok=True)
        clip_paths = []
        voc = getattr(self, '_vocals_path', getattr(self, 'vocals_path', ''))
        if not voc or not os.path.exists(voc):
            from core.cache_manager import CacheManager, Step
            vpath = self.video_path_edit.text().strip()
            if vpath:
                cm = CacheManager(vpath, os.path.join(os.getcwd(), ".cache"))
                voc = cm.get_path(Step.DEMUCS, "vocals_orig.wav")
        if not voc or not os.path.exists(voc):
            QMessageBox.warning(self, "训练音色", "未找到人声文件")
            shutil.rmtree(tmp_dir, ignore_errors=True)
            return
        self.log(f"  人声文件: {os.path.basename(voc)}")
        self.lbl_cache_status.setText("提取音频片段...")
